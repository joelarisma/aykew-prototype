 ...
 *
 * This has a couple of small modifications for eyeQ commented within the code
 *
 * 
 */
(function(window) {
	...
	var _globals = window.GreenSockGlobals || window,
		...
		_split = function(element, vars, allChars, allWords, allLines) {
			...
			//copy all the descendant nodes into an array (we can't use a regular nodeList because it's live and we may need to re-nest things)
			...
			if (lines) {
				...
				element.style.cssText = "display:none;"; //to improve performance, set display:none on the element so that the browser doesn't have to worry about re-flowing or rendering while we're re-nesting things. We'll revert the cssText later.
				...
				for (i = 0; i < lines.length; i++) {
					curLine = lines[i];
// *******
// Modifications for eyeQ
					lineNode = _doc.createElement("div");
					if (linesClass) {
// eyeQ - iterate from 0 rather than 1
// old:					lineNode.className = linesClass + (iterateLine ? i+1 : "");
						lineNode.className = linesClass + (iterateLine ? i : "");
// *******
						}
